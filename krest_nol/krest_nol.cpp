...
//-------------------Дерево-------------------
//Рекурсивное дерево
int tree(int** field, int size, int &count_wins, int current_level, int player, int deep, int revard, int *parametrs) {
	...
	if (current_level == 0) {
		...
		if (count_zeros <= 4) {			
			...
			if (auto_step != -1) {
				...
				loc_revard = tree(zap_field, size, count_wins, current_level + 1, player*(-1), deep - 1, loc_revard, parametrs);
		...
		for (int i = 0; i < count_zeros; i++) {
			if ((win1[i] == false) && (win2[i] == false)) {		//Случай если победитель ещё не найден	
				...
				arr_revard[i] = tree(arr[i], size, count_wins, current_level + 1, player*(-1), deep - 1, arr_revard[i], parametrs);
			}
		}		
...
//Корень дерева
int tree(int** field, int size, int &count_wins, int current_level, int player, int deep, int *parametrs) {
	if ((deep == 0) || (deep > 9)) { return 0; }
	if (current_level == 0) {		
		int res = tree(field, size, count_wins, current_level + 1, player, deep, parametrs);
	...
	while (!win) {
		...
		case '1': {			
			if ((win == false) && (nichya == false)) {
				...
				player = player * (-1);
				if ((win == false)&&(nichya==false)) {						
					int predict = tree(a, size, win_0, 0, player, 7, parametrs); //Новое дерево
		...
		case '2': {
			...
			predict = tree(a, size,win_0,0, player, 7, parametrs);	//новое дерево				
			int end_time = clock();
		...
		case '3': {		
			...
			while (flag)
			{
				if ((win == false) && (nichya == false)) {
					...
					int predict = tree(a, size, win_0, 0, player, 5, param1);
					input_step(predict, a, size, player);
					print(a, size);
					win = logic(a, size, player);
					nichya = logic(a, size);
					...
					if ((win == false) && (nichya == false)) {
						cout << "Player: " << player << endl;
						int predict1 = tree(a, size, win_0, 0, player, 5, param2);
